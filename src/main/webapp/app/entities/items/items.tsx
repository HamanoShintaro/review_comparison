            ...
            <tbody>
              {itemsList.map((items, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {items.reviewReveals ? <Link to={`review-reveals/${items.reviewReveals.id}`}>{items.reviewReveals.id}</Link> : ''}
                  </td>
                  <td>
                    {items.tags
                      ? items.tags.map((val, j) => (
                          <span key={j}>
                            <Link to={`tags/${val.id}`}>{val.id}</Link>
                            {j === items.tags.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
